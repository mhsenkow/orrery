...
console.log('derived record extras');
{
  ...
  ok('Io tidal heat ~2 W/m²', io.tidalHeatWm2?.v > 0.5 && io.tidalHeatWm2?.v < 10, `${io.tidalHeatWm2?.v}`);
  const luna = makeWorldRecord(seedByName('Luna'));
  ok('Moon tidal heat << Io', luna && luna.tidalHeatWm2?.v < io.tidalHeatWm2.v * 0.05, `${luna?.tidalHeatWm2?.v}`);
