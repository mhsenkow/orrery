...
export function fireTick(W, log = null) {
  ...
  if (!isPinnedEarth(W.rule) && !W.rule?.daisyworld) {
    const stride = 97;
    ...
    const rate = 0.03 * (1 + (W.stormyFrac || 0));
    for (let c = off; c < NC; c += stride) {
      ...
      if (rng() < rate * d * d) {
        ...
        if (log) log(W.year, 'fire', c, d, 'Dry lightning · fire started');
      }
    }
  }
