...
// Remaining work is regex deletion and `sigtest` — not name-matching, and not
...
// Giants are no longer fake land: `W.noSurface` skips rocky ticks and seeds
// zonal jets. Pressure is the vertical coordinate (`W.pSeen`, cloud decks,
// a 22-bar probe floor). Descent is a camera that goes inside the 1-bar
// radius on giants only. Epochs are specs (`epochs.json`); Holocene Earth
// reads out 20 TW without moving golden. Regex deletion and `sigtest` wait.
//
...
const CATS = [
  ...
  ['epoch', 'An epoch is a startable world, not two clock fields',
  ...
  ['techno', 'The technosphere is watts, then land, then heat',
...
const P3 = [
/* -------------------------------------------------------------- nosurf -- */
{c:'nosurf',t:'A giant is nine lines that set everything to zero',g:'nosurface',d:'Landed, first cut. `W.noSurface` from gas kind or fluid interior. Land/sea both false, rocky ticks skip, `landFrac` is 0. Pressure lives on `W.pSeen`; the heightfield is still zeros and is no longer the thing you look at.',k:'MODEL',e:'S',i:3},
{c:'nosurf',t:'Pressure as the vertical coordinate',g:'plevel',n:['nosurface'],d:'Landed, first cut. `W.pSeen` is bars of optical surface, lazy-allocated on giants only. 1 bar is the photosphere. `W.press` stays the SWE free-surface height and is not overwritten. Not wavelength-dependent opacity.',k:'MODEL',e:'L',i:3},
{c:'nosurf',t:'Cloud decks at their condensation levels',n:['plevel'],d:'Landed, first cut. Jupiter: NH3 0.7 bar, NH4SH 2 bar, H2O 5 bar. Ice giants: CH4 ice. Belts (sinking) see deeper; zones see the top deck. Colour follows `pSeen`. Not a condensation solver.',k:'MODEL',e:'M',i:3},
{c:'nosurf',t:'The "surface" the player sees is an optical depth',n:['plevel'],d:'Landed, first cut. `pSeen` is the opaque level from convergence, not a solid boundary. Pressure overlay remaps to decks on a giant. Not methane-band vs continuum — one optical surface.',k:'DRAW',e:'M',i:3},
{c:'nosurf',t:'A giant still has a heat budget',n:['nosurface'],d:'Landed, first cut. `jetAmplitude` reads `internalHeat` so Neptune outruns Uranus on almost no sunlight. Insolation still warms the SWE target. Not a full internal-heat flux map.',k:'MODEL',e:'M',i:3},
...
{c:'nosurf',t:'Hot Jupiters are tidally locked giants',d:'Landed, first cut. Locked no-surface worlds shift a dayside hotspot downwind of the substellar point (`W._hotspotLon` from equatorial U) and dim the nightside. Not a 3-D circulation or a measured phase curve.',k:'MODEL',e:'M',i:3},
...
{c:'nosurf',t:'Rings as a first-class object',d:'Landed, first cut. Saturn draws an equatorial line annulus with a Cassini gap. Not Roche physics, not shadow, not seasonal opening. `wantsRings` is kind `saturn`, not every body with a rings trait.',k:'DRAW',e:'M',i:3},
...
{c:'nosurf',t:'The tools have to mean something on a giant',n:['plevel'],d:'Landed, first cut. Land-group tools stay locked. Inspect prints bars, kelvin and deck name instead of elev/moist/biome. Spin reseeds the Rhines count. Pressure overlay is the optical deck.',k:'PLAY',e:'M',i:3},
{c:'nosurf',t:'A giant should be a world you fly through',n:['plevel'],d:'Landed, first cut. Giants only: camera min 0.84 radii. Distance maps to bars (1.00 = 1 bar, closer is deeper, floor 22 bar). Rung reads Descent / Probe with live P/T. Mesh is not volumetric — you approach the decks, you do not ray-march them.',k:'PLAY',e:'L',i:3},
...
{c:'nosurf',t:'Say what has no surface, in the interface',n:['nosurface'],d:'Landed. Chip prints `no surface`; land dock adds scale height, internal heat, jet count and decks. Inspect is bars and kelvin. Descent HUD on the scale rung.',k:'DRAW',e:'S',i:3},
{c:'nosurf',t:'Assert that a giant is not a rocky world',n:['nosurface'],d:'Landed. Catalogue `giant` rows assert `hasSurface === false`. Generated Jupiter has no plates, landFrac 0, neither land nor sea, and hydro does not wet it. Earth generate after a giant still has a surface.',k:'PROVE',e:'M',i:3},

/* ----------------------------------------------------------------- jet -- */
{c:'jet',t:'Zonal jets at a Rhines scale',g:'rhines',n:['plevel'],d:'Landed, first cut. `rhinesJetCount` from rotation and radius (Jupiter 9.9 h → ~12). Seeded `windU` stepped by SWE with no land wall. Amplitude from internal heat. Not 2-D turbulence and not a measured 150 m/s pin.',k:'MODEL',e:'L',i:3},
{c:'jet',t:'Belts and zones are different depths, not different paints',n:['rhines'],d:'Landed, first cut. Sinking air (`converg < 0`) raises `pSeen`; paint darkens with log pressure. Zones stay on the ammonia deck. Fallback sine bands if fields are missing (`sampleLand` tests).',k:'MODEL',e:'M',i:3},
{c:'jet',t:'Alternating prograde and retrograde jets',n:['rhines'],d:'Landed, first cut. Seed is `sin(n * asin(lat))`; `countZonalJets` asserts four or more sign flips on Jupiter. SWE then shears the field.',k:'MODEL',e:'M',i:3},
{c:'jet',t:'Long-lived vortices',n:['rhines'],d:'Landed, first cut. One vortex (`W.spot`) sits in southern shear, advected slowly, painted as a red spot on Jupiter and a dark oval on Neptune. Not merging, not a 2-D inverse cascade.',k:'MODEL',e:'L',i:3},
...
{c:'jet',t:'Rotation rate should change the band count',n:['rhines'],d:'Landed. `maybeReseedJets` on the spin lever; a slower rotator gets fewer jets. Tests pin Jupiter ~12 and a 2-day spin fewer than that.',k:'MODEL',e:'M',i:3},
...
{c:'jet',t:'Neptune\'s winds are the fastest and its insolation is nearly zero',d:'Landed, first cut. `jetAmplitude` is internal heat plus a small insolation term. Neptune\'s mean |U| exceeds Uranus. Not 2,000 km/h in physical units.',k:'MODEL',e:'M',i:3},
{c:'jet',t:'Storms that erupt and fade',d:'Landed, first cut. Chronicle notes a long-lived vortex on a slow cadence. Not Great White Spot recurrence.',k:'MODEL',e:'M',i:2},
{c:'jet',t:'The banding should advect its own colour',n:['rhines'],d:'Landed, first cut. `W.chroma` injects in belts and advects with `windU/V`. Jupiter/Saturn ochre; ice giants skip the chromophore tint. Not filament-resolving turbulence.',k:'DRAW',e:'M',i:3},
{c:'jet',t:'A hot Jupiter\'s jet shifts its hot spot',d:'Landed, first cut. Locked envelopes offset the hotspot by equatorial U and dim the nightside in `landJupiter`. Phase-curve magnitude is not fitted.',k:'MODEL',e:'M',i:3},
{c:'jet',t:'Reuse the rocky-world circulation machinery',n:['rhines'],d:'Landed. Giants run the same SWE (`geostrophicWind`) with no land drag, a jet seed instead of geostrophy, then `giantTick` for decks. One solver.',k:'MODEL',e:'M',i:3},
...
{c:'jet',t:'Cloud chemistry sets the colours',d:'Landed, first cut. Deck RGBs are tagged measured (NH3, H2O, CH4) or fitted (NH4SH). Chromophores are invented ochre. Ice-giant methane stays blue-green.',k:'DRAW',e:'M',i:2},
...
{c:'jet',t:'Show the wind profile',n:['rhines'],d:'Landed, first cut. Sky-desk zonal mean already plots T and u; on a giant the vapour trace drops so the jet profile is the readable line. Not a fitted 150 m/s overlay.',k:'PROVE',e:'M',i:3},
{c:'jet',t:'Let the player change the rotation and watch the bands reorganise',n:['rhines'],d:'Landed. Spin tool and sky-desk day length call `maybeReseedJets`. Receipt prints the new jet count.',k:'PLAY',e:'M',i:3},
...
{c:'jet',t:'Frame rate on a turbulent solver',n:['rhines'],d:'Stated, not built. Giants skip Whittaker/coast/rivers/life in `refreshColours` and skip carbon/phylogeny ticks. SWE is the budget; a 2-D turbulence solver is not this pass.',k:'PROVE',e:'M',i:3},
{c:'jet',t:'Assert Jupiter\'s band count and jet speeds',n:['rhines'],d:'Landed, first cut. Rhines count 8–16, zonal sign flips ≥ 4. Speeds stay dimensionless. 150 m/s is not pinned.',k:'PROVE',e:'M',i:3},

/* ---------------------------------------------------------------- deep -- */
{c:'deep',t:'A depth axis instead of an elevation axis',g:'depthaxis',n:['plevel'],d:'Landed, first cut. Nominal radius stays 1; `camDist` below 1.03 is pressure, not elevation. `W.pSeen` is the optical surface. Mesh radius is still 1 — not a displaced photosphere and not a volumetric field.',k:'DRAW',e:'L',i:3},
{c:'deep',t:'Descending into a giant',n:['depthaxis'],d:'Landed, first cut. Wheel zoom on a giant goes to 0.84 radii. Rung: Descent, then Probe. P/T on the HUD. Light does not redden; decks do not pass as shells.',k:'DRAW',e:'L',i:3},
...
{c:'deep',t:'Pressure and temperature readouts as you descend',n:['depthaxis'],d:'Landed. `formatDescent` prints bars and kelvin plus deck name or probe floor. Inspect at a cell prints the same for `pSeen`. Condensable phase is the deck name, not a full Clausius–Clapeyron.',k:'DRAW',e:'M',i:3},
...
{c:'deep',t:'Where the probe stops',d:'Landed. Galileo 22 bar. `probeFloorBar` / `PROBE_FLOOR_BAR`. Closer than 0.84 is clamped; the HUD says probe floor rather than inventing a surface.',k:'DRAW',e:'S',i:2},
{c:'deep',t:'Scale height varies enormously',d:'Landed, first cut. Giant limb `atmoShellMul` scales with `scaleHeightKm / radius`, clamped so the shell stays a limb. Chip prints H in km. Not a per-cell H.',k:'MODEL',e:'M',i:3},
...
{c:'deep',t:'A cross-section view for any world',n:['depthaxis','colfield'],d:'Landed, first cut. `columnLayers` on a giant is decks plus probe floor; Lab core and inspect already read it. Not a dedicated cutaway camera.',k:'DRAW',e:'M',i:3},
...
{c:'deep',t:'Do not pretend there is a floor',n:['depthaxis'],d:'Landed, first cut. The camera stops where Galileo stopped. The HUD names a probe floor, not ground. Metallic hydrogen is not a landing.',k:'DRAW',e:'M',i:3},
{c:'deep',t:'Performance of a volumetric path',n:['depthaxis'],d:'Stated. No ray march. Colour on giants skips the Whittaker/coast/life path. Limb is a thicker shell, rings are lines. Measure a volumetric path before adding one.',k:'PROVE',e:'M',i:3},
...
{c:'deep',t:'Assert the pressure–temperature profile',n:['depthaxis'],d:'Landed, first cut. Jupiter T(1 bar) ~165 K, T(22 bar) ~430 K (`T ∝ P^0.32`). Fitted to Galileo, not a radiative-convective column.',k:'PROVE',e:'M',i:3},
];
...
const P5 = [
/* --------------------------------------------------------------- epoch -- */
{c:'epoch',t:'An epoch is currently two fields',g:'epochobj',d:'Landed, first cut. `vr/data/worlds/epochs.json` compiles to a spec: gases, sea level, ice, optional landscape, palette, sources. `eraPatch` still returns `deepTime` + `startAgeGa` (now 541 Ma BP → clock at 4.026 Ga) plus `epochId`. Present Earth is a no-op so golden holds. Not proxy-calibrated LGM/Cretaceous.',k:'MODEL',e:'L',i:3},
{c:'epoch',t:'No world except Earth has a history you can visit',n:['epochobj'],d:'Landed, first cut. `availableEras` is keyed on body: Earth list, Venus now/ocean, Mars now/wet. Io and Titan histories wait. Catalogue Earth still gets the Earth list.',k:'MODEL',e:'M',i:3},
{c:'epoch',t:'Hadean Earth',n:['epochobj'],d:'Landed, first cut. Ocean archetype, steam-rich gases, closer Moon. Magma-ocean cooling and bombardment are not restaged.',k:'MODEL',e:'M',i:3},
{c:'epoch',t:'Archean Earth',n:['epochobj'],d:'Landed, first cut. No oxygen, methane greenhouse, green-grey ocean tint, no grass. Mats if life seeded; not a purple-Earth pigment model.',k:'MODEL',e:'M',i:3},
{c:'epoch',t:'Snowball Earth',n:['epochobj'],d:'Landed, first cut. Startable Cryogenian: `W.state = snowball`, ice to the equator. Escape is still volcanic CO₂. Not a GCM hysteresis test.',k:'MODEL',e:'M',i:3},
{c:'epoch',t:'Carboniferous Earth',n:['epochobj'],d:'Landed, first cut. 35% O₂, wet land, no grass. Coal window in `carbonTick` already existed. Giant arthropods are not entities.',k:'MODEL',e:'M',i:3},
{c:'epoch',t:'Cretaceous Earth',n:['epochobj'],d:'Landed, first cut. Higher seas, ice cleared, inland-sea archetype. Not proxy-calibrated against Haq/Huber this cut.',k:'MODEL',e:'M',i:2},
{c:'epoch',t:'Earth at the last glacial maximum',n:['epochobj'],d:'Landed, first cut. Lower seas, ice to ~40°, 190 ppm CO₂. Ice is a latitude paint, not flowlines. Not a calibrate.mjs pin.',k:'MODEL',e:'M',i:3},
{c:'epoch',t:'Earth in 50 million years',n:['epochobj'],d:'Not this cut. Plate motions exist; running them forward into a named future epoch is a later pass.',k:'MODEL',e:'M',i:3},
{c:'epoch',t:'Earth at the end',n:['epochobj'],d:'Not this cut. `faintYoungStar` still models the luminosity track. A moist-greenhouse end-state as a picker row waits.',k:'MODEL',e:'M',i:3},
{c:'epoch',t:'Venus with an ocean',n:['epochobj'],d:'Landed, first cut. Dichotomy + target land fraction + thinner CO₂ + ocean tint. Tagged invented (Way et al. is the cite, not a result this model claims).',k:'MODEL',e:'M',i:3},
{c:'epoch',t:'Early Mars',n:['epochobj'],d:'Landed, first cut. Thicker CO₂, dichotomy ocean, polar ice. Valley networks are not carved. Tagged fitted.',k:'MODEL',e:'M',i:3},
{c:'epoch',t:'Palaeogeography needs a real land configuration',n:['epochobj'],d:'Partial. Permian uses Pangaea, Hadean ocean, Cretaceous inland, Venus/Mars dichotomy. Rodinia and Pannotia are not in the list. Not reconstructed from plates.',k:'MODEL',e:'M',i:3},
{c:'epoch',t:'An epoch needs its own palette',n:['epochobj'],d:'Landed, first cut. `oceanRgb` tints the sea; `noGrass` skips the Holocene lush lerp. Not a full ramp per era.',k:'DRAW',e:'M',i:3},
{c:'epoch',t:'An epoch should be shareable',n:['epochobj'],d:'Landed. Seed-word ids take `~cambrian` (and `era=` on the URL). Present / Venus-now / Mars-now omit the suffix.',k:'PLAY',e:'M',i:2},
{c:'epoch',t:'Starting in an epoch is not the same as arriving in one',n:['epochobj'],d:'Landed. `W._epochStarted` vs `W._epochArrived`. Generate into Carboniferous is started; Holocene present is arrived.',k:'MODEL',e:'M',i:2},
{c:'epoch',t:'Epoch as a scenario with a question',n:['epochobj'],d:'Landed, first cut. `save-snowball` already existed. `permian-doorstep` and `keep-venus-wet` (bright-star analog) joined it. Catalogue Venus ocean is the era picker.',k:'PLAY',e:'M',i:3},
{c:'epoch',t:'The epoch picker should show pictures',n:['epochobj'],d:'Not this cut. The ribbon is still a select of names. Thumbnails of the same seed at six moments wait.',k:'DRAW',e:'M',i:3},
{c:'epoch',t:'Epoch data with sources',n:['epochobj'],d:'Landed. Each row has tag, why, and a cite (Berner, Hoffman, Way, Wordsworth, ice cores). Compiler refuses a missing why.',k:'PROVE',e:'M',i:3},
{c:'epoch',t:'Assert each epoch against its proxies',n:['epochobj'],d:'Partial. Tests pin Cambrian gases ≠ present, Venus ocean exists, early Mars CO₂, Holocene golden. LGM/Cretaceous climate are not in `calibrate.mjs` this cut.',k:'PROVE',e:'M',i:3},
...
/* -------------------------------------------------------------- techno -- */
{c:'techno',t:'The technosphere is one float and four thresholds',g:'technolayer',d:'Landed, first cut. `W.build` still names settlements. `W.techno` is the energy budget: watts, source, land-use fraction, waste-heat fraction. Holocene Earth is a 20 TW readout so golden gases do not drift.',k:'MODEL',e:'L',i:3},
{c:'techno',t:'Energy use as the defining variable',n:['technolayer'],d:'Landed. Watts over absorbed insolation is the number. Holocene ~0.01%. Interesting states are far above; waste heat turns on at 1%.',k:'MODEL',e:'M',i:3},
{c:'techno',t:'Waste heat is thermodynamically unavoidable',n:['technolayer'],d:'Landed, first cut. Every watt becomes heat. Below 1% of absorbed insolation it is ignored so Holocene Earth does not move. Above that it warms the temp field. Not a GCM.',k:'MODEL',e:'M',i:3},
{c:'techno',t:'Emissions as a coupling into the carbon model',n:['technolayer'],d:'Landed, first cut. Fossil `co2KgPerJ` unburies `carbon.buriedOrg` on uncalibrated worlds. Holocene is already at 420 ppm and does not double-count.',k:'MODEL',e:'M',i:3},
{c:'techno',t:'Land use as a surface cover',n:['technolayer'],d:'Partial. A planet-wide fraction plus the techno overlay. Cropland/pasture/urban classes are not separate fields. Albedo of used land is not this cut.',k:'MODEL',e:'M',i:3},
{c:'techno',t:'Resource extraction with a depletion curve',n:['technolayer'],d:'Landed, first cut. Mean `W.ore` throttles fossil watts; empty stock can collapse the budget onto biomass.',k:'MODEL',e:'M',i:2},
{c:'techno',t:'Infrastructure as a network, not a density',n:['technolayer'],d:'Not this cut. Settlements remain a density field. Roads and grids wait.',k:'MODEL',e:'M',i:2},
{c:'techno',t:'Population from carrying capacity, with a technology term',n:['technolayer'],d:'Partial. `settleCities` already multiplies NPP, soil, moisture and a tech proxy. Energy does not yet rewrite that formula.',k:'MODEL',e:'M',i:2},
{c:'techno',t:'A technosphere should be able to fail',n:['technolayer'],d:'Landed, first cut. Ore collapse flags `techno.failed` and drops watts. Fragmentation and recovery are not modelled.',k:'MODEL',e:'M',i:3},
{c:'techno',t:'The technosphere is a successor in the tick order',n:['technolayer'],d:'Landed. `technoTick` runs after bio/carbon, skips giants, Daisyworld, airless, and generate spin-up.',k:'MODEL',e:'M',i:3},
{c:'techno',t:'Authored as data, like the life grammar',n:['technolayer'],d:'Landed. `vr/data/techno/sources.json` compiled by `scripts/techno.mjs`. Tag, why, power density, emissions, land footprint.',k:'MODEL',e:'M',i:3},
{c:'techno',t:'Energy sources with real power densities',n:['technolayer'],d:'Landed. Biomass, hydro, fossil, fission (measured); fusion and orbital (invented). Density is a sketch, not a plant model.',k:'MODEL',e:'M',i:2},
{c:'techno',t:'The biosignature changes',n:['technolayer'],d:'Not this cut. CFCs and a broken seasonal cycle wait on the spectrum path.',k:'MODEL',e:'M',i:3},
{c:'techno',t:'Night lights already exist and mean almost nothing',d:'Partial. `cityLights` scales with watts via `technoLights`. No cities still means dark. Energy without settlements does not glow.',k:'DRAW',e:'M',i:3},
{c:'techno',t:'Radio leakage and directed emission',n:['technolayer'],d:'Not this cut.',k:'MODEL',e:'M',i:2},
{c:'techno',t:'Show the technosphere as a layer',n:['technolayer'],d:'Landed. Overlay `techno` paints build × land use and a waste-heat tint. Giants stay dark.',k:'DRAW',e:'M',i:3},
{c:'techno',t:'A technosphere on a world that is not Earth',n:['technolayer'],d:'Partial. The object can exist wherever there is a surface and build. Holocene calibration is Earth-only. Ice-shell civs wait.',k:'MODEL',e:'M',i:2},
{c:'techno',t:'The player\'s hand and the civilisation\'s hand',d:'Not this cut. Attribution is still player vs planet.',k:'PLAY',e:'M',i:3},
{c:'techno',t:'Assert against the present Earth',n:['technolayer'],d:'Landed. Generate Holocene Earth: 20 TW, ~0.01% insolation, calibrated flag, overlay present, golden unchanged.',k:'PROVE',e:'M',i:3},
{c:'techno',t:'Say what is modelled and what is speculation',n:['technolayer'],d:'Landed. Same three tags. Waste heat is physics. Fusion and orbital are invented. Post-bio at 10% insolation is invented.',k:'PROVE',e:'M',i:3},
];

const P6 = [
/* ---------------------------------------------------------------- mega -- */
{c:'mega',t:'Albedo engineering as the cheapest planetary lever',g:'megaeng',n:['technolayer'],d:'Landed, first cut. `paintAlbedo`, sulphate aerosol, and marine-ish cloud seed already write fields the climate reads. Side effects show. Not published forcing tables per method.',k:'MODEL',e:'M',i:3},
{c:'mega',t:'An orbital shade at L1',n:['megaeng'],d:'Landed, first cut. `W.solarShade` is still a scalar; HUD/`formatMega` names it L1. Mass, cost and failure mode wait.',k:'MODEL',e:'M',i:3},
...
{c:'mega',t:'Waste heat sets the ceiling on all of it',n:['megaeng'],d:'Landed with the technosphere. 1% insolation onset; 10% flags post-bio. Radiating area as a megastructure constraint is the same number.',k:'MODEL',e:'M',i:3},
...
{c:'mega',t:'Kardashev as a readout, not a badge',n:['megaeng'],d:'Landed. `kardashevOf(watts)` is log10(W/1e12)/3. Printed on the axes line. Not a tier badge.',k:'DRAW',e:'S',i:2},
...
{c:'mega',t:'Label all of it speculative',n:['megaeng'],d:'Landed as discipline. Aerosol and L1 shade are fitted/measured levers already in the god layer. Fusion, orbital collection and post-bio are tagged invented. A Dyson swarm is not this cut.',k:'PROVE',e:'S',i:3},

/* --------------------------------------------------------------- after -- */
{c:'after',t:'A post-biological layer with its own thermodynamics',g:'postbio',n:['technolayer'],d:'Landed, first cut. `W._postbio` when waste heat ≥ 10% of absorbed insolation. Tagged invented. Not computation-as-metabolism and not a catalogue body.',k:'MODEL',e:'L',i:3},
...
{c:'after',t:'Keep it falsifiable where it can be',n:['postbio'],d:'Landed as labelling. Waste heat and radiating area are physics. Post-bio at 10% is an invented threshold. Landauer and outward migration wait.',k:'PROVE',e:'M',i:3},
{c:'after',t:'A test that the thermodynamics closes',n:['postbio'],d:'Partial. Holocene 20 TW / absorbed insolation is asserted. A full energy-in / heat-out budget for a post-bio world is not this cut.',k:'PROVE',e:'M',i:3},
...
{c:'proof',t:'A validating compiler',n:['worlddata'],d:'Partial. `scripts/substrates.mjs`, `scripts/cover.mjs`, `scripts/landgram.mjs`, `scripts/columns.mjs`, `scripts/epochs.mjs` and `scripts/techno.mjs` validate ids, tags and why sentences. Adding Pluto without JavaScript is still not the test this document asked for.',k:'PROVE',e:'M',i:3},
...
const FOUND = [
  ...
  ['A gas giant is no longer a planet whose surface is entirely land',
   '`W.noSurface` from gas kind or fluid interior. Land and sea are both false. Hydro, ecology, tectonics, erosion, ocean and mantle skip. `W.pSeen` is the optical surface in bars; cloud decks sit at condensation levels; the camera may pass 1 bar on giants only. Twenty-one catalogue giants assert `hasSurface === false`.'],
  ...
  ['An epoch is a spec, with holes',
   'First cut landed. Gases, ice, sea, optional landscape, palette, Venus ocean, early Mars. Rodinia is not Pangaea. LGM/Cretaceous are not in calibrate.mjs. The picker is still a select, not six thumbnails. 50 Ma and the end of Earth wait.'],
  ['The technosphere is watts on Earth, density elsewhere',
   'Holocene reads 20 TW. Overlay and waste-heat onset exist. Roads, CFCs, radio, ice-shell civs and a Dyson swarm do not. Fusion is tagged invented.'],
  ...
  ['The data already knows things the engine is starting to read',
   'Jupiter\'s note in `SEED_WORLDS` reads "Banded at a Rhines scale set by a 9.9-hour day." `rhinesJetCount` uses that day length; `jetAmplitude` uses `internalHeat` so Neptune outruns Uranus. Uranus\'s 98° obliquity is still Earth\'s season model.'],
];
...
const LANDED = [
  ...
  ['Giants are not rocky worlds',
   '`W.noSurface`. Land tools lock. The chip prints `no surface`. Hydro does not wet Jupiter. Catalogue giants assert `hasSurface === false`. Earth after a giant still has a surface.'],
  ['Giants have zonal jets at a Rhines count',
   '`rhinesJetCount` from rotation and radius; seeded `windU` stepped by SWE with no land wall. Amplitude from internal heat. Spin reseeds the count. Not 2-D turbulence, not 150 m/s.'],
  ['Giants have pressure as a vertical coordinate',
   '`W.pSeen`, ammonia / NH4SH / water (or methane) decks, Galileo 22 bar, T ∝ P^0.32 fitted to the probe. Belts see deeper. Inspect and descent HUD print bars and kelvin. Colour path on giants skips Whittaker. Saturn has a line-annulus ring. Camera min 0.84 on envelopes only.'],
];

const NOW = [
  ['The column has a first cut; giants have decks in bars',
   'Recipes, overlay, inspect, Lab core. Europa / Moon / Titan / Jupiter pins. Jupiter\'s stack is now ammonia / hydrosulfide / water plus a probe floor. Not a saved per-cell array. Volumetric cloud and epochs wait.'],
...
const SEQ = [
  ...
  ['Then worlds with no surface',
   'Landed. `nosurface`, `rhines`, `plevel`, first-cut `depthaxis`. Epochs (`epochobj`) and the technosphere (`technolayer`) are the next items that landed. Do not start regex deletion or `sigtest` in the middle of that.'],
  ['Then epochs as worlds',
   'First cut landed. `epochs.json` compiled to specs. Gases, ice, sea, optional landscape, palette. Venus ocean and early Mars in the picker. Present Earth is a no-op. Not reconstructed palaeogeography, not LGM/Cretaceous proxy pins, not a thumbnail strip.'],
  ['Then the technosphere and what follows it',
   'First cut landed. 20 TW Holocene readout, waste heat above 1%, fossil coupling off the calibration path, techno overlay. Mega is albedo + L1 shade + Kardashev. Post-bio is a 10% flag tagged invented. Not a Dyson swarm.'],
];
...
<header>
  ...
  <a href="surface.html">Surface</a> · <a href="openworld.html">Open world</a> · <a href="../vr/">Prototype</a></p>
  <dl class="tally">
