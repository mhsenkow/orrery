...
{c:'gyres',t:'An actual ocean velocity field',g:'oceanvel',n:['ekman'],d:'`W.oceanU` and `W.oceanV`, prognostic, in the tangent frame. Nothing in `vr/` currently stores where seawater is going. Every remaining item in this category, the whole of the modes category, and half the biology of the coasts is downstream of two `Float32Array(NC)`.',k:'SIM',e:'M',i:3},
{c:'gyres',t:'Sverdrup balance closes the interior',g:'gyre',n:['oceanvel'],d:'βv = curl(τ)/ρ — the interior of every ocean basin moves meridionally in proportion to the curl of the wind stress. Under a three-cell atmosphere this produces subtropical gyres turning one way and subpolar gyres turning the other, without any of them being drawn by hand. It is the cleanest big result in physical oceanography and it costs one line once β exists.',k:'SIM',e:'M',i:3},
{c:'gyres',t:'Western boundary intensification',g:'wbc',n:['gyre'],d:'The return flow cannot be spread across the basin because β is not symmetric, so it concentrates into a narrow, fast jet on the western edge — the Gulf Stream, the Kuroshio, the Agulhas, the Brazil Current. The asymmetry falls straight out of Stommel’s argument once there is a β term and a basin with a coast. This is the item the request named.',k:'SIM',e:'M',i:3},
...
{c:'gyres',t:'Coastlines that steer water instead of stopping it',n:['oceanvel'],d:'A no-normal-flow boundary condition at every land cell, with a free-slip tangential component. Without it, currents either leak through continents or stall against them; with it, the shape of a basin starts to determine the shape of its circulation, which is the whole point of having drawn a coastline.',k:'SIM',e:'M',i:3},
{c:'gyres',t:'Sea surface height as the pressure the currents balance',n:['gyre'],d:'Geostrophic ocean flow follows contours of sea surface height, and SSH is the single most informative field satellites measure — gyres appear as domes, eddies as bumps, the ACC as a staircase. It is also a natural overlay and it makes `W.seaLevel`, currently one global scalar, into a field with structure.',k:'SIM',e:'M',i:2},
...
/* ---------------------------------------------------------------- air -- */
{c:'air',t:'Prognostic pressure and momentum',g:'progatm',n:['vecop'],d:'`W.press[c] = (1 - temp) * 0.55 + elevation * 0.22 + ice * 0.18 - landHeat` is diagnosed from scratch every tick, so the atmosphere has no state and cannot remember anything for even one tick. A shallow-water system — height and two velocity components, stepped forward — is roughly a hundred lines and it is the difference between a picture of circulation and a circulation.',k:'SIM',e:'L',i:3},
{c:'air',t:'Delete the dead wind field',d:'`atmoTick` calls `computeWinds(W)`, which overwrites `windU`/`windV` with three prescribed latitude bands; `advect` then transports temperature, moisture and ash with those; and immediately afterwards `world.js` calls `geostrophicWind(W)`, which overwrites them again. The pressure-derived wind never advects anything, and the band field is computed and discarded every tick on every world.',k:'SIM',e:'S',i:3},
{c:'air',t:'Absolute and potential vorticity',g:'vort',n:['progatm'],d:'ζ + f, conserved on a column that stretches or squashes. Potential vorticity is the single most useful diagnostic in atmospheric dynamics — it is what makes air turn when it crosses a mountain range, what makes a cyclone a coherent object, and what a tropopause fold actually is. Once `curl` exists it is nearly free.',k:'SIM',e:'M',i:3},
...
<header>
  ...
  <h1>Currents</h1>
  <p class="sub">Everything here is supposed to move. The ocean has no velocity field, the
  atmosphere is recomputed from scratch every tick so it can remember nothing, El Niño does not
  exist, magma has no chemistry, and two continents can converge for a billion years without
  ...
  <a href="exoparams.html">Real parameters</a> · <a href="living.html">Alive</a> · <a href="../vr/">Prototype</a></p>
  <dl class="tally">
...
<div class="prose" style="margin-top:56px">
  ...
  <p style="margin-top:16px">The through-line: every fluid in this model is either prescribed or
  absent. The ocean has no velocity field, the atmosphere has no memory, magma has no chemistry,
  and the crust stops responding the moment generation ends — and all four are blocked behind one
  missing object, a tangent frame that tells a cell which way is east.</p>
</div>
