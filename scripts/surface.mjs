...
const CATS = [
  ...
  ['circ', 'Bands should be a result, not an input',
    'Earth has three circulation cells per hemisphere because of rotation rate and the meridional temperature gradient. Pressure and wind are one-layer rotating shallow water; ITCZ/Walker/monsoon sit in the height target. Locked worlds are substellar. Honest daily-mean insolation is what lets high obliquity invert the annual mean.'],
...
{c:'ink',t:'Stroke fade belongs to the overlay clock, not the render clock',d:'Landed. Stroke highlight decays once per sim tick inside `refreshColours`, not from wall-clock `performance.now()`.',k:'DRAW',e:'S',i:2},
...
{c:'belt',t:'Wind is three prescribed bands',d:'Landed. The sine-of-latitude bands and the prescribed trades/westerlies are gone. Wind is rotating shallow water of `W.press`; ITCZ, Walker and monsoon heat the height target. Dead `computeWinds` is deleted.',k:'MODEL',e:'M',i:3},
{c:'belt',t:'A pressure field, so circulation is a solution',g:'pressfield',n:['beltaudit'],d:'Landed. `W.press` is prognostic height in a one-layer SWE (`vr/sim/swe.js`). The heating target still uses temperature, elevation and ice; ITCZ/Walker/monsoon enter η_eq, not extra wind. Cell count still labels the regime from rotation. Not a GCM.',k:'MODEL',e:'L',i:3},
...
{c:'belt',t:'ENSO is applied by longitude sign',d:'Landed. `noteTropicalBasin` flood-fills the largest tropical ocean; west/east is relative to that basin\'s centroid, not world-space `x`. Walker ascent and the rain dipole read `ensoEastness`. The fill itself caches for 32 ticks (or until sea level moves) — it used to rebuild every tick because the key was `W.year`.',k:'MODEL',e:'M',i:2},
...
/* ---------------------------------------------------------------- circ -- */
{c:'circ',t:'A surface pressure field',n:['pressfield'],d:'Landed. Prognostic `W.press` from rotating shallow water; the thermal target is temperature, height and ice. Wind is the SWE velocity, booted from geostrophy of that target.',k:'MODEL',e:'L',i:3},
...
{c:'circ',t:'The jet and its meanders',n:['pressfield'],d:'Landed a first cut. Thermal-wind and fronts share one four-neighbour walk (pressure, temperature, max |ΔT|). Overlay `front` is the same gradient, painted.',k:'MODEL',e:'M',i:2},
{c:'circ',t:'Advect moisture with the wind that exists',n:['vapourfield'],d:'Landed. Moisture and vapour advect with the SWE wind. Dead `computeWinds` is gone. GPU climate still runs CPU SWE so hydro and storms see live wind.',k:'MODEL',e:'M',i:3},
...
{c:'circ',t:'Ocean gyres',d:'Landed as SWE. `oceanU/V` step with wind stress and a land wall; gyres and western intensification have to come from β and geometry. SST still cools under upwelling; ENSO uses basin eastness. Not primitive equations.',k:'MODEL',e:'L',i:3},
...
{c:'circ',t:'Show the circulation, not just its effects',d:'Landed. Overlay `wind`, `vort`, Lab synoptic barbs, and `flowviz.js` all read the SWE. Overlay `current` is the ocean. Fronts are a separate overlay.',k:'DRAW',e:'M',i:2},
...
{c:'circ',t:'Cost and resolution',d:'Landed as a statement, then trimmed. SWE is explicit O(NC), two substeps, inertial advection, free-slip coasts, cached `NBR_E/NBR_N`. River height-sort and ENSO basin fill no longer run every tick; `neighbourEN` and advection flux reuse one slot. N=96 is the Lab budget; a GCM remains out of scope (`briefs/model-limits.md`).',k:'PROVE',e:'M',i:3},
{c:'circ',t:'Bands should survive as an emergent result',n:['zonalviz'],d:'Landed as far as one-layer SWE will go. Earth still bands because of insolation and spin (Ro printed). A locked world is `substellar` with no ITCZ. High obliquity inverts the annual insolation. Not a primitive-equation atmosphere.',k:'MODEL',e:'M',i:3},
];
...
{c:'vapour',t:'Rayleigh depletion along a trajectory',n:['vapourfield'],d:'Landed a first cut. Rain removes local vapour, more on windward slopes (`upslope`). Advected air arriving at the next range is drier. Not a tagged parcel trajectory.',k:'MODEL',e:'M',i:3},
...
{c:'vapour',t:'Evaporation should depend on wind and humidity',d:'Landed. Ocean, lake and land evap all use wind × saturation-deficit against `W.vapour`. Soil `moist` is how much water the land has left to give.',k:'MODEL',e:'S',i:3},
...
{c:'vapour',t:'Soil moisture and atmospheric moisture are different things',d:'Landed a first cut. Clouds read vapour RH. The biosphere still drinks `W.moist`. Whittaker ppt is 82% soil moisture and 18% instantaneous rain, so a wet day can nudge the biome without pretending soil is humidity.',k:'MODEL',e:'M',i:3},
{c:'vapour',t:'A closed water budget with a field in it',n:['vapourfield'],d:'Landed a first cut. Evaporate locally, advect, rain (removing vapour), wet the soil, run off, melt snowpack back into `moist`. Global mass is still the soft `waterMass` constraint; per-cell is the flux chain.',k:'MODEL',e:'M',i:3},
...
{c:'vapour',t:'Seasonal water storage',d:'Landed a first cut. When ice melts it returns water to `W.moist` (spring pulse). Accumulation is still only from precip below the snow line.',k:'MODEL',e:'M',i:2},
{c:'vapour',t:'Groundwater with a residence time',d:'Landed a first cut. The water table decays at 0.992 per tick instead of dumping in a few steps, so a river can still run between storms.',k:'MODEL',e:'M',i:2},
...
{c:'vapour',t:'Atmospheric rivers',n:['vapourfield'],d:'Landed a first cut. `W.ariver` is poleward vapour × wind. Extra rain along those filaments; overlay `ariver` paints them.',k:'MODEL',e:'M',i:2},
{c:'vapour',t:'Fog as a separate thing from cloud',d:'Landed a first cut. `W.fog` is high RH, cool still air, usually coastal. Overlay `fog` and a pale wash on the terrain. Not a radiation-fog temperature profile.',k:'MODEL',e:'M',i:2},
{c:'vapour',t:'The water budget on worlds with no water',d:'Landed. Airless worlds are `cycleMode` none and skip rain entirely (no more 15% water-cycle). Frost worlds keep the phase sketch; methane still runs Titan.',k:'MODEL',e:'M',i:3},
{c:'vapour',t:'Assert the global water balance closes',n:['vapourfield'],d:'Landed. After 24 ticks `waterDrift` stays under 0.4. Soft conservation still bleeds excess into vapour rather than inventing an ocean.',k:'PROVE',e:'M',i:3},
{c:'vapour',t:'Compare precipitation against real Earth',d:'Landed a loose first cut. `calibrateEarth` asserts mean precip in `[0.0005, 0.25]` plus the existing zonal R² cap. Not a GPCC profile.',k:'PROVE',e:'M',i:3},
...
{c:'rain',t:'The lee term looks one cell downwind',d:'Landed a first cut. Lee is copied three cells downwind at 0.55 each step, so a rain shadow is a region rather than a one-cell fringe.',k:'MODEL',e:'M',i:3},
...
{c:'rain',t:'Rain over the ocean matters too',d:'Landed. Ocean precipitation already freshens `oceanSalt`. The rest of stratification is still the mixed-layer sketch.',k:'MODEL',e:'M',i:2},
{c:'rain',t:'Precipitation should reach the surface picture',d:'Landed. The shader darkens wet ground from `precipF`; CPU colour also darkens moist land. Fog is a separate pale wash.',k:'DRAW',e:'M',i:3},
...
{c:'rain',t:'Rain on ice',d:'Landed a first cut. Warm rain multiplies melt (`1 + precip × 1.6`) so a thawing pack does not ignore the rain.',k:'MODEL',e:'M',i:1},
{c:'rain',t:'Cloud shadow on the ground',d:'Landed. The surface shader already multiplies land and water by local `cloudF`. `uCloudShadow` is the disc-mean for the limb term.',k:'DRAW',e:'M',i:3},
{c:'rain',t:'Lightning where the convection is',d:'Landed. Flashes gate on `cloudF × precipF`, not soil moisture. `uStorm` still counts convective cells from clouds × (rain or convergence).',k:'DRAW',e:'S',i:2},
...
{c:'measure',t:'Frame time by stage',d:'Landed. `refreshColours` writes `W._msColour`; the Sky desk prints it. Sim tick time is `W._msSim`. Local view skips the draw when the canvas is hidden.',k:'PROVE',e:'S',i:3},
...
{c:'measure',t:'Deterministic rendering',d:'Landed. Stroke fade decays once per sim tick, not from `performance.now()`, so a frame is a function of world state plus the overlay clock.',k:'PROVE',e:'S',i:3},
...
const LANDED = [
  ...
  ['Wind is a solution to pressure',
   '`pressfield`. Prognostic `W.press` from one-layer SWE. The sine-of-latitude bands and prescribed trades are gone. Overlay mode `vapour` shows the moisture field that those winds now carry.'],
  ...
  ['Earth circulation leftovers are a first-cut sketch, not a GCM',
   'Daily-mean insolation, basin ENSO, fronts, thermal-wind meanders, lake/forest evaporation, Rossby number and a zonal-mean Lab section. One-layer rotating shallow water for air and sea. No elliptic solver, no GCM.'],
...
const NOW = [
  ...
  ['The banding is being replaced by mechanisms, not deleted',
   'Atmospheric water is a per-cell field, advected. Continentality is a BFS from the coast. Biome colour blends by membership. Wind is one-layer shallow water. Insolation is a daily-mean TOA cosine. Locked worlds skip zonal ITCZ. Remaining Earth stripes are whatever Hadley descent the SWE actually produces.'],
...
const SEQ = [
  ...
  ['Build the pressure field',
   '`pressfield`. Landed: one-layer rotating shallow water. True Hadley cells and a GCM remain out of scope.'],
...
function markdown() {
  ...
  L.push('');
  L.push('The short version: **the rectangle is a UI overlay that used to repaint the whole globe** (fixed), the straight edges were an interpolation stencil that clamped at a cube face and a texture atlas that filtered across five seams (fixed), and **the stripes were real** — vegetation, ice, deserts and rainfall used to be painted on latitude bands. Vapour is a field, wind is rotating shallow water, ice follows coldness, rain follows ascent, clouds follow humidity, and insolation is a daily-mean TOA cosine. Remaining Earth structure is Hadley descent and the seasonal hemisphere.');
  L.push('');
...
<header>
  ...
  <a href="worldspace.html">World space</a> · <a href="openworld.html">Open world</a> · <a href="../vr/">Prototype</a></p>
  <dl class="tally">
...
<div class="prose" style="margin-top:56px">
  ...
  no atmospheric water field; vapour is now per-cell, wind is rotating shallow water, and Earth vegetation and
