...
{c:'clock',t:'A biology clock that is not the geology clock',g:'biotime',n:['unpin'],d:'Landed (first cut). `W.dtBio` / `W.bioGen` advance separately; beings accumulate `m.gen` from body mass. Geology still uses `dtYr`.',k:'MODEL',e:'L',i:3},
{c:'clock',t:'Generations, not years',n:['biotime'],d:'Landed (first cut). `m.gen` accumulates per life step from Kleiber body mass. Lineage-level still open.',k:'MODEL',e:'M',i:3},
{c:'clock',t:'Four times is not fast',g:'livespeed',d:'Landed (first cut). `W.lifeSpeed` 1/2/4/8 substeps `agentsTick` inside one climate tick. Frame FF is separate.',k:'MODEL',e:'M',i:3},
{c:'clock',t:'Substep life inside one climate tick',n:['livespeed','biotime'],d:'Landed. `lifeSubsteps()` runs N `agentsTick`s per `simTick`. Ribbon Life dial 1–8×; pinned terra stays 1×.',k:'MODEL',e:'M',i:3},
{c:'clock',t:'Say what the rate is, on screen',n:['livespeed'],d:'Landed (first cut). Ribbon Life 1×–8× plus bio yr/step readout.',k:'SHOW',e:'S',i:3},
...
{c:'being',t:'A drive vector, not a behaviour string',g:'drivevec',n:['energy'],d:'Partial. Hunger, fear, thirst and heat are continuous; pickBehav reads the loudest. Crowding still open.',k:'MODEL',e:'M',i:3},
...
{c:'census',t:'A population book per lineage',g:'popbook',n:['birth','death'],d:'Partial. `W.popBook` counts births, deaths, hunted. Per-lineage books and immigration still open.',k:'MODEL',e:'M',i:3},
...
{c:'census',t:'Assert the accounting',n:['popbook'],d:'Landed (first cut). Suite asserts Δn ≈ births − deaths over a window (immigration still zero).',k:'PROVE',e:'M',i:3},
...
const P2 = [
/* ---------------------------------------------------------------- hunt -- */
{c:'hunt',t:'A hunt that happens on a cell',g:'hunt',n:['entsim','percept'],d:'Landed (first cut). Chase range five cells; cover + defence trait cuts hit chance; carcass and fear on outcome.',k:'MODEL',e:'L',i:3},
...
{c:'hunt',t:'Hunts that fail',n:['hunt'],d:'Landed. Cover, awareness and `TRAITS.defence` reduce hit chance. Misses raise fear.',k:'MODEL',e:'M',i:3},
...
{c:'swarm',t:'A leader, and following',n:['group','memory'],d:'Landed (first cut). Highest-energy member is `leaderId`; others bias toward `leaderCell`.',k:'MODEL',e:'M',i:3},
...
{c:'swarm',t:'A swarm on the globe',n:['group'],d:'Landed (first cut). Groups ≥4 render as pulsing crosses on the sphere; hunt/flee beings tint and pulse from orbit.',k:'SHOW',e:'M',i:3},
{c:'swarm',t:'Migration, along a route',n:['group','memory'],d:'Landed (first cut). Groups store `summerCell`/`winterCell` from lat extremes; `route` flips with `sin(season)`.',k:'MODEL',e:'L',i:3},
...
/* -------------------------------------------------------------- spread -- */
{c:'spread',t:'A colonisation front with a shape',g:'front',d:'Landed (first cut). `lifeFront` + `lifeFlux` fields; edge = sparse beside lush; ice/deep sea/arid damp the rim. Overlay Life front.',k:'MODEL',e:'M',i:3},
{c:'spread',t:'Dispersal with a distance',g:'disperse',n:['front'],d:'Landed (first cut). Wind/flow long hops seed two cells away on thrive; barriers cut land↔sea and arid jumps. Fat-tail kernel still open.',k:'MODEL',e:'M',i:3},
{c:'spread',t:'Barriers, from the terrain that is already there',n:['front'],d:'Landed (partial). Ice, deep sea and arid cells suppress front strength and dispersal. Mountain/strait corridors still open.',k:'MODEL',e:'M',i:3},
...
{c:'spread',t:'A front rendered as a front',n:['front'],d:'Landed (first cut). Life front overlay paints the leading rim brighter than the interior; flux overlay shows gain/loss.',k:'SHOW',e:'M',i:3},
...
{c:'patch',t:'Draw the act, not the actor',g:'tileact',n:['tilelife','drivevec'],d:'Landed (first cut). Local view draws hunt/flee/forage/tend/surface marks; gait speeds up on flee/hunt; birth/death/hunt sparks.',k:'SHOW',e:'M',i:3},
...
{c:'patch',t:'A gait',d:'Landed (partial). Flee/hunt raise gait frequency and amplitude; rest stills. Displacement-tied phase still open.',k:'SHOW',e:'M',i:2},
{c:'patch',t:'Eating, visibly',n:['tileact'],d:'Landed (first cut). Forage mark plus a bite dot when hunger is low; scavengers already go to carcasses. Dedicated eat duration still open.',k:'SHOW',e:'M',i:3},
...
{c:'patch',t:'A hunt, framed',n:['hunt','tileact'],d:'Landed (first cut). `huntGlance` prefers a live hunter cell, then hunt sparks. Local hunt wedge marks the act.',k:'SHOW',e:'M',i:3},
...
/* --------------------------------------------------------------- globe -- */
{c:'globe',t:'A behaviour overlay',g:'behovl',n:['preyfield'],d:'Landed (partial). Behaviour, fear, carcass, trail, life front and flux overlays. Birth/death rate fields still open.',k:'SHOW',e:'M',i:3},
{c:'globe',t:'Population change, signed',n:['popbook'],d:'Landed (partial). Life flux overlay is signed biomass change per cell. Population-book-driven birth/death map still open.',k:'SHOW',e:'S',i:3},
{c:'globe',t:'Groups and swarms on the sphere',g:'globeswarm',n:['group'],d:'Landed (first cut). `swarmMarks` + globe crosses for herds ≥4 and carcasses; entity tint/pulse for hunt/flee from orbit.',k:'SHOW',e:'M',i:3},
...
{c:'globe',t:'The colonisation front, animated',n:['front'],d:'Landed (first cut). Life front overlay + flux. Shader pulse still open.',k:'SHOW',e:'M',i:3},
...
/* ------------------------------------------------------------- gaiamind -- */
{c:'gaiamind',t:'Four if-statements are the whole autopilot',g:'gaiapolicy',d:'Landed (first cut). Replaced by drive-aware policy that calls `setOrbit` / `injectGas` / `injectAerosol` with quiet receipts.',k:'MODEL',e:'M',i:3},
...
{c:'gaiamind',t:'Do not let the autopilot cheat',n:['gaiapolicy'],d:'Landed (first cut). Acts through `setOrbit`, `injectGas`, `injectAerosol` with throttled receipts. Budget costs still open.',k:'MODEL',e:'M',i:3},
...
{c:'gaiamind',t:'Regulation that fails visibly',n:['gaiapolicy'],d:'Landed (first cut). `W.gaiaFailed` when tips are tripped and climate is past verbs; Lab shows overwhelmed.',k:'MODEL',e:'M',i:3},
