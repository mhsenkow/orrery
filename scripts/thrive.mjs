...
const CATS = [
  ...
  ['outside', 'The visible life is not in the simulation',
    '`agentsTick()` is called from the render loop in `vr/main.js:1248`, not from `simTick`. So every being on the globe is outside the simulation: absent from `runHeadless`, absent from `serializeRun` — a save restores the phylogeny and every genome and restores not one individual — and absent from the test suite, where exactly **1** of 542 assertions mentions an agent, a city or a behaviour. The layer the player actually watches is the only layer nothing checks.'],
...
/* ------------------------------------------------------------- outside -- */
{c:'outside',t:'Move beings inside the simulation tick',g:'entsim',d:'`agentsTick()` is called from `vr/main.js:1248` inside the render loop, immediately after `simTick()`. Move the call into `simTick` so beings are part of the world rather than part of the view. Everything else in this category is blocked on it and it is a small change with a large blast radius, so it goes first and alone.',k:'MODEL',e:'M',i:3},
{c:'outside',t:'Beings in the headless runner',n:['entsim'],d:'`runHeadless` imports `world.js` and never `agents.js`, so every measurement this project has ever taken of its own biosphere excluded the visible layer. Once beings are in the tick they are in the probe, and claims about population become checkable.',k:'PROVE',e:'S',i:3},
{c:'outside',t:'Beings in the save',g:'entsave',n:['entsim'],d:'`serializeRun` version 5 stores the phylogeny and every genome, and not one individual. Save the population — cell, kind, age, lineage, name, state — and a reload restores the world the player was actually looking at rather than a repopulated approximation of it.',k:'MODEL',e:'M',i:3},
...
{c:'outside',t:'One assertion, today',n:['entsim'],d:'Of 542 `ok(` calls in `vr/sim/test.mjs`, one mentions an agent, a city or a behaviour. Write the first five now: respawn produces beings on a living world, produces none on an airless one, every being sits on a valid cell, no being is on a cell it cannot survive, and the count is stable across two identical runs.',k:'PROVE',e:'S',i:3},
...
{c:'outside',t:'Frame-rate independence, asserted',n:['entsim'],d:'With `agentsTick` on the render loop, behaviour steps per simulated year depend on the machine. Assert that two runs with the same seed and the same number of `simTick` calls produce the same population, which is only possible once beings are inside the tick.',k:'PROVE',e:'S',i:3},
...
{c:'outside',t:'A probe for the living layer',g:'liveprobe',n:['entsim'],d:'The audit numbers in this document came from a twenty-line script that generates a world, ticks it, and prints beings by kind, behaviours, deaths, max age, cities and lineages. Commit it as `scripts/thrive-probe.mjs` and every claim here becomes a number that moves.',k:'PROVE',e:'S',i:3},

/* --------------------------------------------------------------- being -- */
{c:'being',t:'Energy, so a life has a cost',g:'energy',n:['entsim'],d:'The first missing quantity and the one everything else hangs off. A being has a store, spends it on movement, thermoregulation and growth, gains it by eating, and dies at zero. Kleiber already gives the metabolic rate from body mass and `plan.size` is already expressed from the genome, so the constants are not invented.',k:'MODEL',e:'M',i:3},
{c:'being',t:'A drive vector, not a behaviour string',g:'drivevec',n:['energy'],d:'`m.behav` is one of five strings chosen by a probability roll. Replace it with a small set of continuous drives — hunger, thirst, fear, heat stress, urge to breed, crowding — each a number that grows and is discharged by an action. Behaviour then falls out of which drive is loudest, which is both cheaper to reason about and richer to watch.',k:'MODEL',e:'M',i:3},
...
/* -------------------------------------------------------------- census -- */
{c:'census',t:'Birth, so a population can grow',g:'birth',n:['energy'],d:'Nothing is ever born. `topUpEntities` writes new beings into free slots from `life[c]` — measured, 252 → 489 beings over 1.7 Gyr with zero deaths and every survivor at max age. A birth needs a parent, a cost in energy, a place and a time, and it is the single change that turns a rendering into a population.',k:'MODEL',e:'M',i:3},
{c:'census',t:'Death, from something',g:'death',n:['energy'],d:'Measured `dead: 0` over 300 ticks. Death from starvation when energy hits zero, from predation, from exposure, from disease and from old age — each logged with its cause so the mortality profile of a world is legible.',k:'MODEL',e:'M',i:3},
...
const P2 = [
/* ---------------------------------------------------------------- hunt -- */
{c:'hunt',t:'A hunt that happens on a cell',g:'hunt',n:['entsim','percept'],d:'`updateFoodWeb` decides that lineage A eats lineage B and applies a Lotka–Volterra term to a single planet-wide `censusPop`. A hunt is a predator on a cell, a prey within sense range, a pursuit that can fail, an energy transfer on success and a carcass. All of it needs the per-cell being buckets `rebuildBuckets` already builds.',k:'MODEL',e:'L',i:3},
{c:'hunt',t:'Predation pressure as a field',g:'preyfield',n:['hunt'],d:'Kills per cell per unit time, decayed. It is the field that makes prey avoid a place, makes a predator return to one, drives the landscape of fear, and gives the globe its first behavioural overlay. One `Float32Array` and an accumulate.',k:'MODEL',e:'M',i:3},
...
{c:'hunt',t:'Hunts that fail',n:['hunt'],d:'A success probability from the difference in speed, size, cover and the prey being aware. If every hunt succeeds, predators are a mortality constant. If they fail most of the time, the whole system gains the texture it should have and predator energy becomes a real constraint.',k:'MODEL',e:'M',i:3},
{c:'hunt',t:'A carcass, and everything that comes to it',g:'carcass',n:['hunt'],d:'`W.detritus` already accumulates from a biomass rain. A discrete carcass is a resource at a place: scavengers arrive, a decomposer bloom follows, nutrients enter the soil at that cell. It is the cheapest way to make a food web feel like a web rather than a graph.',k:'MODEL',e:'M',i:3},
...
{c:'hunt',t:'Assert one kill',n:['hunt'],d:'A predator and a prey on adjacent cells, one tick, one hunt: prey population falls by one, predator energy rises by the prey mass times an efficiency, a carcass appears, a chronicle line is written. Five assertions and the whole category has a floor.',k:'PROVE',e:'S',i:3},
...
{c:'swarm',t:'Groups that split and merge',n:['group'],d:'Fission–fusion when a group exceeds a size or a patch runs out. It gives the visible population a rhythm of its own, independent of the slow biomass field, and it is a few conditions on an existing record.',k:'MODEL',e:'M',i:2},
...
{c:'swarm',t:'Trails, and the field that carries them',n:['memory'],d:'A shared, decaying per-cell field written by movement and read as attraction. It is the cheapest known route to emergent structure — paths, foraging columns, worn tracks — and `noteWear` already writes exactly this shape of field for a different purpose.',k:'MODEL',e:'M',i:3},
...
/* --------------------------------------------------------------- guild -- */
{c:'guild',t:'A trophic field, not three multiplications',g:'guildfield',d:'`W.trophic` is `{nppMean, ×0.1, ×0.01, ×0.3}` and `W.herbivore`/`W.carnivore` follow from it, so a rainforest cell and an ice cell carry identical herbivore load. Producers, herbivores, carnivores and decomposers need to be per-cell — four `Float32Array`s — and `redox.js` already proves the pattern with `guildDens`.',k:'MODEL',e:'L',i:3},
...
/* ------------------------------------------------------------- gaiamind -- */
{c:'gaiamind',t:'Four if-statements are the whole autopilot',g:'gaiapolicy',d:'`gaiaPolicyTick` reads `meanTemp` and `gases.CO2` and applies four threshold nudges to solar and CO₂. Meanwhile `gaiaTick` computes resilience, seven tipping elements with hysteresis, rate stress, feedback gain, a Medea score and habitability-versus-inhabitance. A rich state vector exists and the controller reads two scalars of it.',k:'MODEL',e:'M',i:3},
{c:'gaiamind',t:'A controller that reads the whole state',n:['gaiapolicy'],d:'Resilience, tipping proximity, rate stress and biosphere trend, weighted into an objective. It is still a small amount of arithmetic and it is the difference between a thermostat and something that appears to have a view about the planet.',k:'MODEL',e:'M',i:3},
{c:'gaiamind',t:'Say what it is optimising',n:['gaiapolicy'],d:'Maximum biomass, maximum diversity, maximum stability and maximum complexity are different objectives that produce visibly different planets. Naming the objective is what makes a planetary controller honest rather than mystical.',k:'MODEL',e:'M',i:3},
{c:'gaiamind',t:'Act before the tipping point, not after',n:['gaiapolicy'],d:'`TIPPING` gives seven elements with thresholds and hysteresis, so proximity is computable. A controller that anticipates is qualitatively different to watch from one that reacts, and the data it needs is already on `W.tips`.',k:'MODEL',e:'M',i:3},
...
{c:'gaiamind',t:'Three named Gaia dispositions',n:['gaiapolicy'],d:'Gardener, which maximises life; regulator, which minimises variance; experimenter, which perturbs to see what happens. Selectable, visibly different, and each a different weighting of the same objective — which teaches what a control objective is.',k:'PLAY',e:'M',i:3},
...
{c:'gaiamind',t:'Assert the controller helps',n:['gaiapolicy'],d:'Same seed, same forcing, controller on and off: assert the objective it names is measurably better with it on. An autopilot nobody has measured is a set of four thresholds somebody guessed.',k:'PROVE',e:'M',i:3},
...
{c:'prove',t:'The metrics that must move',g:'livemetric',n:['aliveassert'],d:'Occupied fraction, mean life, lineage count, distinct visible sprites, births and deaths per Myr, groups, settlements, settlement stages, front speed, trophic levels occupied. Eleven numbers, computed headlessly, committed as a baseline. The open-world pass proved that a committed baseline is the difference between a measurement and a guard.',k:'PROVE',e:'M',i:3},
...
{c:'prove',t:'A save round-trip for the population',n:['entsave'],d:'Save, load, assert every being, group and settlement came back identical. The genome round-trip test already exists and is the model for this one.',k:'PROVE',e:'M',i:3},
{c:'prove',t:'A generate-reset test for the living layer',n:['entsim'],d:'The world-space pass found a vent field carrying 173 vents between planets. `ENT` is a module singleton, `_idSeq` is never reset, and `_head`/`_next` persist. Assert that generating twice leaves nothing from the first world.',k:'PROVE',e:'S',i:3},
...
{c:'prove',t:'Print the scoreboard on every run',n:['livemetric'],d:'The eleven metrics, at the end of every headless run, in one line. `runHeadless` already prints a structured summary and the biosphere fields are already computed. Making the number visible is most of what makes it get better.',k:'PROVE',e:'S',i:3},
];
